...
const Toolbar = ({ initialData, preview }: IToolbar) => {
  ...
  <div className="pl-[54px] group relative">
   {!!initialData.icon && !preview && (
    <div className="flex items-center gap-x-2 group/icon pt-6">
     ...
     <Button
      ...
      size={"icon"}
     >
      <X className="size-4" onClick={onRemoveIcon} />
     </Button>
    </div>
   )}
   {!!initialData.icon && preview && (
   ...
   {isEditing && !preview ? (
    <TextAreaAutosize
     ...
     className="text-5xl bg-transparent font-bold break-words text-[#3F3F3F] dark:text-[#CFCFCF] resize-none"
    />
   ) : (
    <div
     onClick={enableInput}
